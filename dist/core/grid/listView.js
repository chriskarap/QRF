System.register(['../sculpt/Sculpt', '../constants', '../math', '../time', '../eventEmitter', '../rodinEvent'], function (_export, _context) {
    "use strict";

    var Sculpt, CONST, Vector3, Time, EventEmitter, RodinEvent;
    return {
                ...
                updateTargetPositions() {
                    ...
                    for (let i = 0; i < pHeight; i++) {
                        ...
                        for (let j = 0; j < pWidth; j++) {
                            ...
                            updated.push(index);
                            const elem = this.getElement(index);
                            if (elem) missingRows[i] = false;else continue;
                    ...
                    for (let i in removed) {
                        if (this.getElement(removed[i])) this.hide(this.getElement(removed[i]), removed[i], 0);
                    }
                    for (let i in updated) {
                        if (this.getElement(updated[i])) this.show(this.getElement(updated[i]), updated[i], 0);
                    }
