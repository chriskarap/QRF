System.register(['../error', '../eventEmitter', '../utils', '../rodinEvent', '../constants', '../math', '../animation', '../loader', '../messenger'], function (_export, _context) {
    "use strict";

    var ErrorBadValueParameter, ErrorPluginAlreadyInstalled, EventEmitter, string, RodinEvent, CONST, Vector3, Euler, Quaternion, AnimationPlugin, Loader, messenger;
    return {
        setters: [function (_error) {
            ErrorBadValueParameter = _error.ErrorBadValueParameter;
            ErrorPluginAlreadyInstalled = _error.ErrorPluginAlreadyInstalled;
        }, function (_eventEmitter) {
            EventEmitter = _eventEmitter.EventEmitter;
        }, function (_utils) {
            string = _utils.string;
            ...
            AnimationPlugin = _animation.AnimationPlugin;
        }, function (_loader) {
        ...
        execute: function () {
            ...
            const GAMEPAD_EVENTS = [CONST.GAMEPAD_HOVER, CONST.GAMEPAD_HOVER_OUT, CONST.GAMEPAD_BUTTON_DOWN, CONST.GAMEPAD_BUTTON_UP, CONST.GAMEPAD_BUTTON_CHANGE, CONST.GAMEPAD_BUTTON, CONST.GAMEPAD_MOVE];
             ...
             */
            class Sculpt extends EventEmitter {
                constructor(args, deferReadyEvent) {
                    ...
                    this.on(CONST.READY, () => {
                        ...
                        if (pendingElements.size === 0) messenger.post(CONST.ALL_SCULPTS_READY, {});
                    });
                    ...
                    this.on(CONST.READY, () => {
                        ...
                        if (!!args.url || !!args.threeObject) {
                            for (let i = 0; i < this._threeObject.children.length; i++) {
                                if (!this._threeObject.children[i].Sculpt) {
                ...
                set parent(parent) {
                    if (parent === null) {
                        ...
                        if (this.parent) this.parent.remove(enforce, this);
                        this._parent = null;
                        return;
                    }
                    if (parent.isSculpt) {
                ...
                on(channel, cb) {
                    ...
                    if (GAMEPAD_EVENTS.indexOf(channel) !== -1) {
                        ...
                        if (this.isReady) {
                            ...
                            for (let i = 0; i < this._threeObject.children.length; i++) Sculpt.raycastables.push(this._threeObject.children[i]);
                        } else this.on(CONST.READY, () => {
                            Sculpt.raycastables.push(this._threeObject);
                            for (let i = 0; i < this._threeObject.children.length; i++) Sculpt.raycastables.push(this._threeObject.children[i]);
                        });
                    }
