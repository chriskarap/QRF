System.register(['./Transport', '../constants', '../messenger', '../initializer', '../device'], function (_export, _context) {
    "use strict";

    var Transport, CONST, messenger, RODIN_ID, device;
    return {
        setters: [function (_Transport) {
            ...
            RODIN_ID = _initializer.RODIN_ID;
        }, function (_device) {
        ...
        execute: function () {
                    ...
                    switch (true) {
                        ...
                        case Array.isArray(packet.destination):
                            for (let i of packet.destination) {
                                if (PostMessageTransport.parentId === i) receivers.push(PostMessageTransport.parent);else if (PostMessageTransport.children[i]) receivers.push(PostMessageTransport.children[i]);else {
            ...
            _export('PostMessageTransport', PostMessageTransport);
            ...
            messenger.on(CONST.ALL_SCULPTS_READY, (data, transport) => {
                if (transport !== postMessageTransport) messenger.post(CONST.ALL_SCULPTS_READY, {
                    destination: CONST.PARENT
                }, postMessageTransport);
            });
            ...
            messenger.on(CONST.ENTER_VR_SUCCESS, (data, transport) => {
                if (transport !== postMessageTransport) messenger.post(CONST.ENTER_VR_SUCCESS, { destination: CONST.PARENT }, postMessageTransport);
            });

            messenger.on(CONST.EXIT_VR_SUCCESS, (data, transport) => {
                if (transport !== postMessageTransport) messenger.post(CONST.EXIT_VR_SUCCESS, { destination: CONST.PARENT }, postMessageTransport);
            });
        }
    };
});