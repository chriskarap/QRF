System.register(['./argumentScheme/ClassType', './argumentScheme/NumberType', './argumentScheme/StringType', './argumentScheme/ArrayType', './argumentScheme/AnyType', './argumentScheme/BoolType', './argumentScheme/FunctionType', '../error'], function (_export, _context) {
    "use strict";

    var ClassType, NumberType, StringType, ArrayType, AnyType, BoolType, FunctionType, ErrorInstantiationFailed, ErrorProtectedMethodCall, ErrorArgumentLoop, ErrorNoDefaultValue, ErrorArgumentType, ErrorUndefinedReference;


    function enforce() {}

    return {
        setters: [function (_argumentSchemeClassType) {
            ClassType = _argumentSchemeClassType.ClassType;
        }, function (_argumentSchemeNumberType) {
            NumberType = _argumentSchemeNumberType.NumberType;
        }, function (_argumentSchemeStringType) {
            StringType = _argumentSchemeStringType.StringType;
        }, function (_argumentSchemeArrayType) {
            ArrayType = _argumentSchemeArrayType.ArrayType;
        }, function (_argumentSchemeAnyType) {
            AnyType = _argumentSchemeAnyType.AnyType;
        }, function (_argumentSchemeBoolType) {
            BoolType = _argumentSchemeBoolType.BoolType;
        }, function (_argumentSchemeFunctionType) {
            FunctionType = _argumentSchemeFunctionType.FunctionType;
        }, function (_error) {
            ErrorInstantiationFailed = _error.ErrorInstantiationFailed;
            ErrorProtectedMethodCall = _error.ErrorProtectedMethodCall;
            ErrorArgumentLoop = _error.ErrorArgumentLoop;
            ErrorNoDefaultValue = _error.ErrorNoDefaultValue;
            ErrorArgumentType = _error.ErrorArgumentType;
            ErrorUndefinedReference = _error.ErrorUndefinedReference;
        }],
        execute: function () {
            class AScheme {
                constructor() {
                ...
                static handleDefault(e, res, defaultValue) {
                    ...
                    if (AScheme.isReference(e, defaultValue)) {
                        ...
                        if (res[defaultValue]) return res[defaultValue];
                ...
                static handleReferenceTree(e, res, scheme, reference, been = []) {
                    ...
                    if (res.hasOwnProperty(reference)) return res[reference];
                ...
                static validate(args, scheme, returnArray = false) {
                    ...
                    if (args.constructor === Array && args.length === 1 && typeof args[0] === 'object' && args[0].constructor === Object) args = args[0];
                    ...
                    if (args.constructor == Array) {
                        ...
                        for (let i = 0; i < references.length; i++) {
                            if (res.hasOwnProperty(references[i])) continue;
