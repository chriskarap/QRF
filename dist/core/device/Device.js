System.register(['../eventEmitter', '../constants', '../error', '../messenger', '../transport', '../rodinEvent'], function (_export, _context) {
    "use strict";

    var EventEmitter, CONST, ErrorUnknownDevice, messenger, localTransport, RodinEvent;
    return {
        setters: [function (_eventEmitter) {
            ...
            ErrorUnknownDevice = _error.ErrorUnknownDevice;
        }, function (_messenger) {
            messenger = _messenger.messenger;
        }, function (_transport) {
            localTransport = _transport.localTransport;
        ...
        execute: function () {
            ...
            class Device extends EventEmitter {
                ...
                get iOSVersion() {
                    ...
                    if (!!window.indexedDB) return CONST.IOS8PLUS;

                    if (!!window.SpeechSynthesisUtterance) return CONST.IOS7;

                    if (!!window.webkitAudioContext) return CONST.IOS6;

                    if (!!window.matchMedia) return CONST.IOS5;

                    if (!!window.history && 'pushState' in window.history) return CONST.IOS4;
                }
            ...
            messenger.on(CONST.VR_DISPLAY_PRESENT_CHANGE, (data, transport) => {
                if (transport === localTransport && device._isVR !== data) {
                    device._isVR = data;
                    if (device._isVR) device.emit(CONST.ENTER_VR, new RodinEvent(_this));else device.emit(CONST.EXIT_VR, new RodinEvent(_this));
                }
            });
