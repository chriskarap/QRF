...
export class Sky extends R.Sculpt {
    constructor(params, sunSphere, light, distance = 70){
        ...
        // Expose variables
        super(skyMesh);
        this.effectController = params;
        this.uniforms = skyUniforms;
        this.sunSphere = sunSphere;
        this.light = light;
        this.distance = distance;
        this._constUpdate = false;
        R.Scene.preRender(()=> {
            if( this._constUpdate ) this.update();
        });
        R.Scene.add(sunSphere);
    }
    ...
    update() {
        ...
        this.sunSphere.position.x = this.distance*0.8 * Math.cos(phi);
        this.sunSphere.position.y = this.distance*0.8 * Math.sin(phi) * Math.sin(theta);
...
const skyShader = {
    ...
    vertexShader: [
        ...
        '	vSunfade = 2.0 - clamp( 1.0 - exp( ( sunPosition.y / 450000.0 ) ), 0.0, 1.0 );',
